...
ORDER BY
 ...
 create a temporary table and count row number from latest dates.
Put a check on this row number to get you only those with values <4. and Enjoy!

WITH t AS (select customer_id, order_id, order_date, 
ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY order_date DESC) as serial_num FROM Orders)

SELECT t1.name as customer_name, t.customer_id, t.order_id, t.order_date FROM t 
LEFT JOIN Customers as t1 USING (customer_id) WHERE serial_num < 4 
ORDER BY customer_name, customer_id, order_date DESC
